...
const help = `Usage: node scripts/daily-settlement-worker.mjs [--once] [options]
Recover persisted DAILY settlement intents using the compiled backend service.
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    const arg = args[i]
    if (seen.has(arg)) { process.exitCode = 2; console.error('Invalid daily settlement worker arguments. Use --help.'); return }
    seen.add(arg)
    ...
    if (!spec || !value || !/^\d+$/.test(value) || !Number.isSafeInteger(Number(value)) || Number(value) < spec[1] || Number(value) > spec[2]) {
      process.exitCode = 2; console.error('Invalid daily settlement worker arguments. Use --help.'); return
    }
  ...
  let prisma
  try {
    const service = require('../dist/services/dailySettlementRecovery.js')
    ;({ prisma } = require('../dist/services/prisma.js'))
    const normalized = service.dailySettlementWorkerOptions(options)
    do {
      try {
        const result = await service.runDailySettlementBatch(normalized, stop.signal)
        console.log(JSON.stringify({ event: 'daily_settlement_batch', ...result }))
        if (once) process.exitCode = service.dailySettlementExitCode(result)
      } catch {
        console.error('Daily settlement worker unavailable; persisted work remains recoverable.')
    ...
    process.exitCode = 1
    console.error('Daily settlement worker unavailable. Check the build and database configuration.')
  } finally {
